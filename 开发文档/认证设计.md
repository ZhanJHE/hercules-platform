...
> **落地状态（2026-09-10）**：✅ 网关已上线（独立进程验签 + 透传头注入 + 剥离伪造头），应用内 JWT 过滤器保留为**双层校验第二层**；黑名单仍在应用层（网关层查询待补）。前端（阶段 G）已按本契约实现：accessToken 仅内存、refreshToken 存 localStorage 静默续期、401 单飞刷新并发合并。
