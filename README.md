...
测试：后端 113 个用例、前端 17 个用例，全部通过（`mvn test`、`npm test`）。
...
| [项目讲解.html](./开发文档/项目讲解.html) | 项目怎么分层、有哪些模块，以及四类请求（查询/选课/登录/对话）一路都经过了什么 | 想快速看懂整体怎么走的 |
...
| [Agent经验.md](./开发文档/Agent经验.md) | 本环境/项目里反复出现的坑与避免方法 | AI 动手前扫一眼 |
...
- 自动化测试：后端 113 个、前端 17 个，都不依赖本机中间件和网络
